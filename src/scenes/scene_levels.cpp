...
Scene_Levels::Scene_Levels(GameData* gameData) : Scene(gameData) {
    ...
    background->colorComponent = new Component_Color(sf::Color(20, 20, 20));
    ...
    for (int i=0; i<10; i++) {
        ...
        m_levelButtons[i]->colorComponent = new Component_Color(sf::Color(34, 139, 34));
        ...
        m_levelLabels[i]->colorComponent = new Component_Color(sf::Color(200,200,200));
    ...
    sf::FloatRect bounds = getTextSize("Level 1", font, 40);
    ...
    m_scrollBackground->transformComponent = new Component_Transform(
            sf::Vector2f(640.0f+150.0f, 300.0f - bounds.size.y / 2),
            sf::Vector2f(1.0f, 1.0f), 0.0f);

    m_scroll = m_entityManager.create("Dynamic");
    m_scroll->colorComponent = new Component_Color(sf::Color(200,200,200));
...
void Scene_Levels::update() {
    ...
    for (int i=0; i<10; i++) {
        ...
        m_levelButtons[i]->transformComponent->position.y = 300.0f + 50.0f * (i-m_firstVisibleLevel);
        m_levelLabels[i]->transformComponent->position.y = 300.0f + 50.0f * (i-m_firstVisibleLevel);
    }

    m_scroll->transformComponent->position.y = m_scrollBackground->transformComponent->position.y + (m_firstVisibleLevel / 4.0f) * 45;
